---
title: "COMPRESSOR_Governance"
author: "Corey Scholes"
affiliation: "EBM Analytics"
version: 3.0
type: "website"
number-sections: true
number-depth: 3
date: "2026-Mar-10"
date-modified: "2026-May-07"

bibliography: CompressorGov references.bib

editor:
  visual
format:
  html:
    toc: true
    toc-expand: 2
    number-sections: true
    code-fold: true
    
execute: 
  echo: false
  warning: false
  message: false
---

# Preamble

The following analysis is a report on the activity, quality and data contained in the COMPRESSOR registry, which is described in the [registry wiki](https://sites.google.com/ebma.com.au/compressor-registry/home?authuser=0).

Analysis packages were loaded initially into the *R* environment.

```{r}
#| label: load-packages

if (!require("pacman")) install.packages("pacman")
pacman::p_load(# Load required packages
  "fuzzyjoin",
  "ggforce",
  "ggdag",
  "ggmap",
  "geosphere",
  "dplyr",
  "flextable",
  "litedown",
  "grateful",
  "modelsummary",
  "quantreg",
  "readr",
  "knitr",
  "cardx",
  "forcats",
  "gargle",
  "googledrive",
  "googlesheets4",
  "openxlsx2",
  "tidyverse",
  "tidymodels",
  "lubridate",
  "consort",
  "gtsummary",
  "survival",
  "ggplot2",
  "ggdist",
  "ggfortify",
  "ggsurvfit",
  "mice",
  "marginaleffects",
  "naniar",
  "quantreg",
  "broom",
  "epoxy",
  "broom.helpers",
  "stringr"
  )
  
  
```

Access to the COMPRESSOR datasets was pre-authorised.

```{r}
#| label: auth2
#| echo: false

options(
  gargle_oauth_cache = ".secrets",
  gargle_oauth_email = TRUE
)

googledrive::drive_auth(
  cache = ".secrets", 
  email = TRUE
  )
```

A function was generated to retrieve files using the *googledrive* package, to call on later in the analysis for processing data imports.

```{r}
#| label: folder
#| echo: false

base_folder_id1 <- "1c08mZXSxqhJx5IniJ1ur3YpWpgWXUoIO"


```

```{r}
get_specific_snapshot <- function(folder_name, base_folder_id = base_folder_id1) {
  tryCatch({
    # Check if the folder exists in the base directory
    folder <- googledrive::drive_ls(as_id(base_folder_id), pattern = paste0("^", folder_name, "$"))
    
    if(nrow(folder) == 0) {
      stop(paste("Folder", folder_name, "not found"))
    }
    
    # Find the snapshot file in the specified folder
    snapshot_file <- googledrive::drive_ls(
      folder$id, 
      pattern = "Registry data snapshot\\.xlsx$"
    )
    
    if(nrow(snapshot_file) == 0) {
      stop("No snapshot file found in specified folder")
    }
    
    # Return both pieces of information as a list
    return(list(
      snapshot = snapshot_file,
      folder_name = folder$name
    ))
    
  }, error = function(e) {
    stop(paste("Error finding specified snapshot:", e$message))
  })
}
```

Data was retrieved from live database tables. Source files were specified and stored as global variables to call on in further functions.

```{r}
#| label: live-table
#| echo: false

SheetIDs <- list(
DbSS = "https://docs.google.com/spreadsheets/d/1ZqqVDOy2Gsq1E0mKAuQokfNyccpNUt4x0EJC156T90Q/edit"
)

# Treatments Masterlist - Timestamp is > Registry Start date (gsuite)

```

```{r}
#| label: read-tables

# Authenticate for sheets using the same token
gs4_auth(token = drive_token())



ComplicTable <- googlesheets4::read_sheet(
  ss = SheetIDs$DbSS,
  sheet = "Complications",
  col_names = TRUE,
  col_types = "cccTciicicccccccccccccccccDccD",
  skip = 1
  )


#To match to acctData
PatientTable <- googlesheets4::range_read(
  ss = SheetIDs$DbSS,
  sheet = "Patient",
  range = "A10:Q",
  col_names = FALSE,
  col_types = "DccccDcccDcicccDi"
  )


Patient_Col <- c(
  "PatientCreationDate",
  "PatientID",
  "LastName",
  "FirstName",
  "AlternateID",
  "DateOfBirth",
  "Sex",
  "RegistryStatus",
  "RegistryStatusNotes",
  "DateRegistryStatus",
  "NotificationMethod",
  "NoTreatmentRecords",
  "Email",
  "Phone",
  "PatientRegistrationStatus",
  "DatePatientRegistration",
  "TrueNoTreatmentRecords"
)

colnames(PatientTable) <- Patient_Col


TreatTable <- googlesheets4::range_read(
  ss = SheetIDs$DbSS,
  sheet = "Treatment", 
  range = "A7:Z",
  col_names = FALSE
  )

TreatTableCol <- googlesheets4::range_read(
  ss = SheetIDs$DbSS,
  sheet = "Treatment", 
  range = "A1:Z1",
  col_names = FALSE
  )

 colnames(TreatTable) <- TreatTableCol


```

A static registry snapshot was retrieved and formatted based on the fixed date of preparation of the snapshot (10-Mar-2026).

```{r}

CurrentDate <- as.character("20260507")

# Authenticate for sheets using the same token
gs4_auth(token = drive_token())

# To get a snapshot from a specific folder (e.g., "20230415")
specific_snapshot <- get_specific_snapshot("20260507")


```

```{r}
#| execute: true

# The snapshot coming out of the framework is over filtering the records and I dont know yet where to make the fix. I think it could be a structural issue. I think I will just bypass the snapshot for now and read in the sheets raw.

temp_file1 <- tempfile(fileext = ".xlsx")
drive_download(
  file = specific_snapshot$snapshot$id,
  path = temp_file1,
  overwrite = TRUE
)

# Correction to reset back to excel origin
DaysDiff <- as.numeric(as.duration(interval(ymd("1899-12-30"), ymd("1970-01-01"))),"days")


Snapshot <- openxlsx2::wb_to_df(
  temp_file1,
  sheet = "Combined",
  colNames = TRUE,
  detectDates = TRUE
  ) |> mutate(
    PatientID = stringr::str_split_i(TreatmentID,"\\.",1),
    SurgicalTreatment2 = case_when(
      stringr::str_detect(str_to_lower(EBMAComment),"surgery recommended") & SurgicalTreatment == "Non-Surgical" ~ "Surgery recommended",
      .default = SurgicalTreatment
    ),
    Sex2 = if_else(
      stringr::str_detect(str_to_lower(Sex), "f"),
      "Female",
      "Male"
    ),
    Retrospective = if_else(
      stringr::str_detect(str_to_lower(AnalysisLabel), "retrospective", negate = TRUE) | is.na(AnalysisLabel),
      "No",
      "Yes"
    ),
    Planned = if_else(
      stringr::str_detect(str_to_lower(AnalysisLabel), "plan"),
      "Yes",
      "No"
    ),
    Hardware = str_extract(str_to_lower(AnalysisLabel), "\\b[a-z]{1,5}\\b"),
    TreatmentType2 = if_else(
    stringr::str_detect(TreatmentType, "Revision"),
    "Revision",
    "Primary"
),
AgeAtInitialExam = as.numeric(AgeAtInitialExam),
AgeAtTreatment = as.numeric(AgeAtTreatment)
  ) |> unite(
  col = "CombID",
  sep = ".",
  c("PatientID","AffectedSide"),
  remove = FALSE
)



STROBEInput <- openxlsx2::wb_to_df(
  temp_file1,
  sheet = "Strobe_Input",
  colNames = TRUE,
  detectDates = TRUE
  )

```

# Context

COMPRESSOR (Complex Reconstructive Surgery Outcomes Registry) is a clinical quality registry supporting the practices of one surgeon in Sydney, New South Wales. It has been in operation over two distinct periods, with upper and lower limb tumour cohorts, as well as tumour of the pelvis and reintervention arthroplasty cohorts of the hip and knee. The periods that the registry has been actively collecting PROMs;

1.  25-Feb-2022 to 29-Sep-2022

2.  27-Mar-2025 to 2-Oct-2025

3.  15-Feb-2026 to Present

It should be noted that collection of PROMs has been heavily restricted for this registry due to concerns over appropriateness to follow up patients at high-risk of mortality or other contraindications for completing scores. These restrictions have only recently been lifted, and only then for cases receiving hardware components of interest (Ossis, Stryker GMRS-MRH).

#Snapshot validation

```{r}
# Identify dropped records
dropped_records <- TreatTable |> filter(
  lubridate::ymd(DateTreatmentRecordCreation) <= ymd(CurrentDate)
) |> mutate(
  TreatmentID = as.character(
    TreatmentID
    )
  ) |> anti_join(
      Snapshot, by = "TreatmentID"
      )

# Row count summary
epoxy(
  "TreatTable (live):   {nrow(TreatTable)} records",
  "Snapshot (pipeline): {nrow(Snapshot)} records",
  "Dropped records:     {nrow(dropped_records)} records",
  .sep = "\n"
)
```

# Data Preparation

```{r}
#| label: proms-setup

PROMHOOS12 <- Snapshot |>
  dplyr::select(
    TreatmentID,
    starts_with("HOOS12_Summary_TotalScore"),
    starts_with("EligibleAt"),
    -EligibleAtIntraop
  ) |>
  rename_with(
    ~ .x |>
      str_replace("^EligibleAtx", "EligibleAt_") |>   # EligibleAtx3months -> EligibleAt_3months
      str_replace("^EligibleAtP", "EligibleAt_P"),     # EligibleAtPreop    -> EligibleAt_Preop
    starts_with("EligibleAt")
  ) |>
  pivot_longer(
    cols = !TreatmentID,
    names_to = c(".value", "TimePoint"),
    names_pattern = "^(.+?)_(Preop|\\d+months)$",
    values_drop_na = TRUE
  ) |> mutate(
  TimePoint = factor(TimePoint, levels = c("Preop","3months","6months","12months","24months", "60months", "120months"), ordered = TRUE, exclude = NA),
  HOOS12SummaryTotal = as.numeric(HOOS12_Summary_TotalScore)
) |> dplyr::filter(
  EligibleAt == "Yes",
  !is.na(TimePoint)
) |> left_join(
  Snapshot |> dplyr::select(
    TreatmentID,
    SurgicalTreatment2,
    TreatmentType2,
    TreatmentStatus,
    RegistryCohortName,
    AnalysisLabel
  ),
  by = "TreatmentID"
) |> unite(
    "TimePointID",
    c("TreatmentID","TimePoint"),
    sep = ".",
    na.rm = FALSE,
    remove = FALSE
    )

PROMKOOS12 <- Snapshot |>
  dplyr::select(
    TreatmentID,
    starts_with("KOOS12_Summary_TotalScore"),
    starts_with("EligibleAt"),
    -EligibleAtIntraop
  ) |>
  rename_with(
    ~ .x |>
      str_replace("^EligibleAtx", "EligibleAt_") |>   # EligibleAtx3months -> EligibleAt_3months
      str_replace("^EligibleAtP", "EligibleAt_P"),     # EligibleAtPreop    -> EligibleAt_Preop
    starts_with("EligibleAt")
  ) |>
  pivot_longer(
    cols = !TreatmentID,
    names_to = c(".value", "TimePoint"),
    names_pattern = "^(.+?)_(Preop|\\d+months)$",
    values_drop_na = TRUE
  ) |> mutate(
  TimePoint = factor(TimePoint, levels = c("Preop","3months","6months","12months","24months", "60months", "120months"), ordered = TRUE, exclude = NA),
  KOOS12SummaryTotal = as.numeric(KOOS12_Summary_TotalScore)
) |> dplyr::filter(
  EligibleAt == "Yes",
  !is.na(TimePoint)
) |> left_join(
  Snapshot |> dplyr::select(
    TreatmentID,
    SurgicalTreatment2,
    TreatmentType2,
    TreatmentStatus,
    RegistryCohortName,
    AnalysisLabel
  ),
  by = "TreatmentID"
) |> unite(
    "TimePointID",
    c("TreatmentID","TimePoint"),
    sep = ".",
    na.rm = FALSE,
    remove = FALSE
    )

PROMMSTS <- Snapshot |>
  dplyr::select(
    TreatmentID,
    starts_with("MSTSLower_Total"),
    starts_with("EligibleAt"),
    -EligibleAtIntraop
  ) |>
  rename_with(
    ~ .x |>
      str_replace("^EligibleAtx", "EligibleAt_") |>   # EligibleAtx3months -> EligibleAt_3months
      str_replace("^EligibleAtP", "EligibleAt_P"),     # EligibleAtPreop    -> EligibleAt_Preop
    starts_with("EligibleAt")
  ) |>
  pivot_longer(
    cols = !TreatmentID,
    names_to = c(".value", "TimePoint"),
    names_pattern = "^(.+?)_(Preop|\\d+months)$",
    values_drop_na = TRUE
  ) |> dplyr::filter(
   EligibleAt == "Yes",
   TimePoint != "Preop",
   !is.na(TimePoint)
 ) |> mutate(
  TimePoint = factor(TimePoint, levels = c("Preop","3months","6months","12months","24months", "60months", "120months"), ordered = TRUE, exclude = NA),
) |> left_join(
   Snapshot |> dplyr::select(
     TreatmentID,
     SurgicalTreatment2,
     RegistryCohortName,
     TreatmentType2,
     AnalysisLabel
   ),
   by = "TreatmentID"
 ) |> unite(
    "TimePointID",
    c("TreatmentID","TimePoint"),
    sep = ".",
    na.rm = FALSE,
    remove = FALSE
    ) 


PROMVR12 <- Snapshot |> dplyr::select(
  TreatmentID,
  starts_with("VR12PCS"),
  starts_with("VR12MCS"),
  starts_with("EligibleAt"),
  -EligibleAtIntraop
) |> rename_with(
    ~ .x |>
      str_replace("^EligibleAtx", "EligibleAt_") |>   # EligibleAtx3months -> EligibleAt_3months
      str_replace("^EligibleAtP", "EligibleAt_P"),     # EligibleAtPreop    -> EligibleAt_Preop
    starts_with("EligibleAt")
    ) |> mutate(
  across(matches("PCS|MCS"), ~as.numeric(.x))
) |> pivot_longer(
  cols = !TreatmentID,
  names_to = c(".value","TimePoint"),
  names_sep = "_",
  values_drop_na = TRUE
) |> mutate(
  TimePoint = factor(TimePoint, levels = c("Preop","3months","6months","12months","24months", "60months", "120months"), ordered = TRUE, exclude = NA),
) |> dplyr::filter(
  EligibleAt == "Yes",
  !is.na(TimePoint)
) |> left_join(
  Snapshot |> dplyr::select(
    TreatmentID,
    SurgicalTreatment2,
    RegistryCohortName,
    AnalysisLabel,
    TreatmentType2
  ),
  by = "TreatmentID"
) |> unite(
    "TimePointID",
    c("TreatmentID","TimePoint"),
    sep = ".",
    na.rm = FALSE,
    remove = FALSE
    )

PROMBPI <- Snapshot |> dplyr::select(
  TreatmentID,
  starts_with("BPISFSeverityScore"),
  starts_with("EligibleAt"),
  -EligibleAtIntraop
) |> rename_with(
    ~ .x |>
      str_replace("^EligibleAtx", "EligibleAt_") |>   # EligibleAtx3months -> EligibleAt_3months
      str_replace("^EligibleAtP", "EligibleAt_P"),     # EligibleAtPreop    -> EligibleAt_Preop
    starts_with("EligibleAt")
    ) |> pivot_longer(
  cols = !TreatmentID,
  names_to = c(".value","TimePoint"),
  names_sep = "_",
  values_drop_na = TRUE
) |> mutate(
  TimePoint = factor(TimePoint, levels = c("Preop","3months","6months","12months","24months", "60months", "120months"), ordered = TRUE, exclude = NA),
) |> dplyr::filter(
  EligibleAt == "Yes",
  !is.na(TimePoint)
) |> left_join(
  Snapshot |> dplyr::select(
    TreatmentID,
    SurgicalTreatment2,
    RegistryCohortName,
    TreatmentType2,
    AnalysisLabel
  ),
  by = "TreatmentID"
) |> unite(
    "TimePointID",
    c("TreatmentID","TimePoint"),
    sep = ".",
    na.rm = FALSE,
    remove = FALSE
    )

```

```{r}
#| label: proms-table-helper

# Helper: builds a Primary | Revision merged tbl_summary
# with timepoints as columns and median (IQR), n per cell
#
# Arguments:
#   data       - long-format PROM dataframe with TreatmentType2 and TimePoint
#   score_var  - unquoted column name of the score variable
#   score_label - string label for the score row
#   timepoints - character vector of timepoint levels to include

build_prom_table <- function(data, score_var, score_label, timepoints) {
  
  score_var <- rlang::ensym(score_var)
  
  data |>
    dplyr::filter(
      TimePoint %in% timepoints,
      !is.na({{ score_var }}),
      TreatmentType2 %in% c("Primary", "Revision")
    ) |>
    dplyr::mutate(
      TimePoint = factor(TimePoint, levels = timepoints)
    ) |>
    dplyr::group_by(TimePoint, TreatmentType2) |>
    dplyr::summarise(
      Median = median({{ score_var }}, na.rm = TRUE),
      Q1     = quantile({{ score_var }}, 0.25, na.rm = TRUE),
      Q3     = quantile({{ score_var }}, 0.75, na.rm = TRUE),
      n      = sum(!is.na({{ score_var }})),
      .groups = "drop"
    ) |>
    dplyr::mutate(
      Cell = dplyr::if_else(
        n > 0,
        glue::glue("{round(Median,1)} ({round(Q1,1)}, {round(Q3,1)}), n={n}"),
        NA_character_
      )
    ) |>
    dplyr::select(TimePoint, TreatmentType2, Cell) |>
    tidyr::pivot_wider(
      names_from  = TreatmentType2,
      values_from = Cell
    ) |>
    dplyr::mutate(Score = score_label) |>
    dplyr::relocate(Score, TimePoint)
}
```

## Base cohort (complic-cohort-base)

```{r}
#| label: complic-cohort-base

ImplantCohort <- Snapshot |>
  dplyr::filter(
    SurgicalTreatment2 == "Surgical",
    !is.na(RegistryCohortName),
    RegistryCohortName != "General"
  ) |>
  dplyr::select(
    TreatmentID,
    CombID,
    DateTreatment,
    TreatmentStatus,
    TreatmentStatusNotes,
    RegistryCohortName,
    AnalysisLabel
  )
```

```{r}
#| label: complic-table2

ComplicTable2 <- ComplicTable |>
  dplyr::select(
    ComplicationID,
    TreatmentID,
    ComplicationOccurrence,
    DateOfOccurrence,
    DateReoperation,
    ComplicationNature,
    ComplicationTreatmentOffered
  ) |>
  left_join(
    ImplantCohort |>
      dplyr::select(TreatmentID, DateTreatment, RegistryCohortName),
    by = "TreatmentID"
  ) |>
  dplyr::filter(!is.na(RegistryCohortName)) |>
  dplyr::mutate(
    Term3 = stringr::str_to_lower(ComplicationNature),
    Mortality = if_else(
      stringr::str_detect(Term3, "died|death|deceas|mortality"),
      "Mortality", "No"
    ),
    Infection1 = if_else(
      stringr::str_detect(Term3, "(implant.+retention)|(periprosthetic.+infect)|(prosthe\\w+.+infect)|suppress|(communication.+prosthesis)|(sinus.+arthroplasty)|((stump|cavity).+infect)"),
      "InfectionDeep", "No"
    ),
    Infection2 = if_else(
      stringr::str_detect(Term3, "superficial|abscess|cellulitis|(surgical site)|wound.+infect|(skin.+tear)") &
        stringr::str_detect(Term3, "superficial wound dehiscence", negate = TRUE) &
        Infection1 == "No",
      "InfectionSuperficial", "No"
    ),
    Infection3 = if_else(
      stringr::str_detect(Term3, "(urinary.+infect)|diverticulitis|pneumon|pyrexia|(abdominal.+collection)") &
        Infection1 == "No",
      "InfectionSyst", "No"
    ),
    Tumour1 = if_else(
      stringr::str_detect(Term3, "carcinoma|recurr|tumour|metasta|margin|chemo"),
      "Tumour", "No"
    ),
    Loosening = if_else(
  stringr::str_detect(Term3, "loosen|fixation failure") &
    stringr::str_detect(Term3, "stem|component|prosthes|implant|femoral|tibial|patella|pubic|pelvic|acetabul"),
  "Loosening", "No"
),
   Hardware = if_else(
  stringr::str_detect(Term3, "irritat|explant|broke|device|cable|screw|\\basis\\b|wire|plate|grip") &
    Loosening == "No",
  "Hardware", "No"
),
    Instability = if_else(
      stringr::str_detect(Term3, "instab|sublux|disloc"),
      "Dislocation", "No"
    ),
    Neurological = if_else(
      stringr::str_detect(Term3, "nerve|palsy|radiculopathy|foot drop|weakness"),
      "Neurological", "No"
    ),
    Fracture = if_else(
      stringr::str_detect(Term3, "fract"),
      "Fracture", "No"
    ),
    Wound = if_else(
      stringr::str_detect(Term3, "dehiscence|wound|stitch|ooze|necrot") &
        Infection1 == "No" & Infection2 == "No",
      "Wound", "No"
    ),
    Thrombosis = if_else(
      stringr::str_detect(Term3, "dvt|embo|\\bpe\\b|occlus|thromb"),
      "Thrombosis", "No"
    )
  ) |>
  dplyr::rowwise() |>
  dplyr::mutate(
    Other = if_else(
      any(stringr::str_detect(
        c_across(c(Mortality, Infection1, Infection2, Infection3, Tumour1,
                   Loosening, Hardware, Instability, Neurological,
                   Fracture, Wound, Thrombosis)),
        "No", negate = TRUE
      )),
      "No", "Other"
    ),
    Tumour2 = case_when(
      Tumour1 == "Tumour" & stringr::str_detect(Term3, "debulk|recurr|margin|pelvis") ~ "TumourLocal",
      Tumour1 == "Tumour" & stringr::str_detect(Term3, "chest|myeloma|lung|breast|pulmonary|clavicle") ~ "TumourSystemic",
      .default = "No"
    )
  ) |>
  dplyr::ungroup()
```

```{r}
#| label: complic-table2a

ComplicTable2a <- ComplicTable2 |>
  dplyr::filter(
    ComplicationTreatmentOffered == "Surgical") |>
  dplyr::select(-DateOfOccurrence) |>
  dplyr::rename(DateOfOccurrence = "DateReoperation") |>
  dplyr::mutate(
    ImplantRemoval = if_else(
      stringr::str_detect(Term3, "explant"),
      "ImplantRemoval", "No"
    ),
    Amputation = if_else(
      stringr::str_detect(Term3, "amput*|hindquarter"),
      "Amputation", "No"
    ),
    Reoperation = if_else(
      Amputation == "No" & ImplantRemoval == "No",
      "Reoperation", "No"
    ),
    ComplicationOccurrence = "Yes - Clinician Confirmed"
  ) |> dplyr::select(
    ComplicationID, TreatmentID, ComplicationOccurrence,
    DateOfOccurrence, Term3, RegistryCohortName,
    Reoperation, ImplantRemoval, Amputation, DateTreatment
  ) |>
  bind_rows(ComplicTable2 |> dplyr::filter(
    str_detect(Term3, "test", negate = TRUE)
  )) |>
  dplyr::relocate(Reoperation:Amputation, .after = Tumour2) |>
  dplyr::mutate(
    across(Mortality:Amputation, ~replace_na(.x, "No"))
  )
```

```{r}
#| label: complic-table3

category_order <- c(
  "Other", "Wound", "Hardware", "Fracture", "Neurological", "Dislocation",
  "Loosening", "Thrombosis", "TumourSystemic", "TumourLocal",
  "InfectionSyst", "InfectionSuperficial", "InfectionDeep",
  "Reoperation", "Amputation", "ImplantRemoval", "Mortality", "Censored"
)

ComplicTable3 <- ComplicTable2a |>
  pivot_longer(
    cols = c(
      Mortality, ImplantRemoval, Reoperation,
      Infection1, Infection2, Infection3,
      Amputation, Tumour2, Thrombosis, Loosening,
      Instability, Neurological, Fracture, Hardware, Wound, Other
    ),
    cols_vary = "slowest",
    names_to = "Category_Name",
    values_to = "Category_Value",
    values_drop_na = FALSE
  ) |>
  dplyr::filter(Category_Value != "No") |>
  dplyr::group_by(TreatmentID) |>
  dplyr::arrange(dplyr::desc(DateOfOccurrence), .by_group = TRUE) |>
  dplyr::distinct(
    pick(TreatmentID, DateOfOccurrence, Category_Value, DateTreatment),
    .keep_all = TRUE
  ) |>
  dplyr::mutate(
    OccurDelay = as.numeric(
      as.duration(interval(ymd(DateTreatment), ymd(DateOfOccurrence))),
      "weeks"
    ),
    Intraop = if_else(OccurDelay == 0, "Yes", "No")
  ) |>
  dplyr::filter(DateOfOccurrence < ymd(CurrentDate)) |>
  dplyr::ungroup()
```

```{r}
#| label: complic-master

ComplicCensor <- ImplantCohort |>
  dplyr::filter(!(TreatmentID %in% ComplicTable3$TreatmentID)) |>
  dplyr::mutate(
    ComplicationOccurrence = "No",
    DateOfOccurrence = ymd(CurrentDate),
    Category_Value = "Censored"
  )

ComplicMaster <- bind_rows(
  ComplicTable3 |>
    dplyr::mutate(Category_Value = as.character(Category_Value)),
  ComplicCensor |>
    dplyr::select(TreatmentID, CombID, RegistryCohortName,
                  ComplicationOccurrence, DateTreatment, DateOfOccurrence, Category_Value)
) |>
  dplyr::mutate(
    Category = factor(Category_Value, levels = category_order),
    SurgicalManagement = if_else(
      ComplicationOccurrence == "Yes - Clinician Confirmed" &
        Category_Value %in% c("Reoperation", "ImplantRemoval", "Amputation"),
      "Surgical",
      if_else(
        ComplicationOccurrence == "Yes - Clinician Confirmed",
        "Non-surgical", "None"
      )
    )
  ) |>
  dplyr::group_by(TreatmentID) |>
  dplyr::arrange(DateOfOccurrence, .by_group = TRUE) |>
  dplyr::mutate(
    RowNum = row_number(),
    DateOfOccurrence1 = if_else(
      RowNum > 1 & DateOfOccurrence == lag(DateOfOccurrence),
      DateOfOccurrence + days(RowNum),
      DateOfOccurrence
    ),
    DurationStop = as.numeric(
      as.duration(interval(ymd(DateTreatment), ymd(DateOfOccurrence1))),
      "weeks"
    ),
    DurationStart = if_else(RowNum > 1, lag(DurationStop), 0),
    DurationDiff  = DurationStop - DurationStart
  ) |>
  dplyr::ungroup() |>
  dplyr::filter(DurationDiff > 0)

ComplicEvents   <- ComplicMaster |> dplyr::filter(Category != "Censored")
ComplicEndpoints <- ComplicMaster |> dplyr::filter(Category == "Censored")
```

```{r}
#| label: complic-summary-prep

category_display <- c(
  "Wound", "Hardware", "Fracture", "Neurological", "Dislocation",
  "Loosening", "Thrombosis", "TumourSystemic", "TumourLocal",
  "InfectionSyst", "InfectionSuperficial", "InfectionDeep",
  "Reoperation", "Amputation", "ImplantRemoval", "Mortality", "Other"
)

# Pivot events to wide, then add any missing category columns as "No"
complic_pivot <- ComplicEvents |>
  dplyr::filter(Category %in% category_display) |>
  dplyr::distinct(TreatmentID, Category) |>
  dplyr::mutate(present = "Yes") |>
  tidyr::pivot_wider(
    names_from  = Category,
    values_from = present,
    values_fill = list(present = "No")
  )

missing_cats <- setdiff(category_display, names(complic_pivot))
if (length(missing_cats) > 0) {
  complic_pivot[missing_cats] <- "No"
}

ComplicWide <- ImplantCohort |>
  dplyr::select(TreatmentID, RegistryCohortName, AnalysisLabel) |>
  left_join(
    ComplicEvents |>
      dplyr::summarise(
        AnyComplication = "Yes",
        AnySurgical     = if_else(any(SurgicalManagement == "Surgical"), "Yes", "No"),
        .by = TreatmentID
      ),
    by = "TreatmentID"
  ) |>
  dplyr::mutate(
    AnyComplication = replace_na(AnyComplication, "No"),
    AnySurgical     = replace_na(AnySurgical, "No")
  ) |>
  left_join(complic_pivot, by = "TreatmentID") |>
  dplyr::mutate(
    across(any_of(category_display), ~replace_na(.x, "No"))
  )
```

# Recruitment Flow by Region

Flowcharts as per STROBE [@Vandenbroucke2007] and RECORD [@Benchimol2015] guidelines were generated for treatments enrolled into the Registry. Followup was set to eligibility at *any postoperative timepoint*.

```{r}


STROBEFlow2 <- STROBEInput |> 
  # Filter out rows with missing TreatmentID
  dplyr::filter(!is.na(TreatmentID)) |> 
  # Join with relevant columns from Snapshot
  dplyr::left_join(
    Snapshot |> 
      dplyr::select(
        TreatmentID,
        CombID,
        DateInitialExamination,
        EligibleAtPreop,
        EligibleAtx3months,
        EligibleAtx6months,
        EligibleAtx12months,
        EligibleAtx60months,
        EligibleAtx120months,
        SurgicalTreatment2
      ) |> 
      # Convert all date columns to date format
      dplyr::mutate(
        across(contains("Date"), lubridate::ymd)
      ),
    # Explicitly specify join column
    by = "TreatmentID"
  ) |> dplyr::mutate(
    TreatmentStatus2 = if_else(
          TreatmentStatus == "No further followup" & is.na(DateTreatment),
          "Archived",
          TreatmentStatus
  ),
  EligibleFU = if_else(
   EligibleAtx3months == "Yes" | EligibleAtx6months == "Yes" | EligibleAtx12months == "Yes" | EligibleAtx60months == "Yes" | EligibleAtx120months == "Yes",
    "Yes",
    "No"
  )
  )

```

```{r}



STROBEFlow3 <- STROBEFlow2 |> dplyr::mutate(
    exclusion1 = case_when(
    is.na(SurgicalTreatment2) ~ "Not a surgical treatment",
    SurgicalTreatment2 == "Surgical" & stringr::str_detect(RegistryStatus,"Opt-out") ~ "Patient Opt-Out",
    SurgicalTreatment2 == "Surgical" & is.na(RegistryCohortName) ~ "No cohort",
    SurgicalTreatment2 == "Surgical" ~ NA_character_,
    .default = "Not a surgical treatment"
  ),
  followup = if_else(
    is.na(exclusion1),
    TreatmentID,
    NA_character_
  ),
  lost_followup = case_when(
    is.na(exclusion1) & TreatmentStatus2 == "Failed" & (ymd(DateStatusChange) < ymd(CurrentDate)) ~ "Procedure failure",
    is.na(exclusion1) & TreatmentStatus2 == "No further followup" & (ymd(DateStatusChange) < ymd(CurrentDate)) ~ "Treatment Progression End",
    is.na(exclusion1) & EligibleFU == "No" ~ "Not eligible for followup"
  ),
  mitt = if_else(
    !is.na(followup) & is.na(lost_followup),
    TreatmentID,
    NA_character_
)
) |> dplyr::rename(
  trialno = "TreatmentID",
  arm3 = "RegistryCohortName"
)

```

```{r}
#| label: fig-strobe-cohort1
#| fig-cap: "Flowchart of extraction and followup of surgical sample from the Registry - 1"


STROBEPlot1 <- consort_plot(
  data = STROBEFlow3 |> dplyr::filter(
    arm3 == "TumourLowerLimb" | arm3 == "ReinterventionTKA" | arm3 == "TumourPelvis"
  ),
 orders = c(
   trialno = "Population",
   exclusion1 = "Ineligible",
   trialno = "Baseline Analysis",
   arm3 = "Cohort",
   lost_followup = "Excluded",
   mitt = "Followup Analysis"
   ),
 side_box = c(
   "exclusion1",
   "lost_followup"
   ),
 allocation = "arm3",
 cex = 0.6
)

knitr::knit_print(STROBEPlot1)

```

```{r}
#| label: fig-strobe-cohort2
#| fig-cap: "Flowchart of extraction and followup of surgical sample from the Registry - 2"


STROBEPlot2 <- consort_plot(
  data = STROBEFlow3 |> dplyr::filter(
    arm3 == "TumourUpperLimb" | arm3 == "ReinterventionTHA" | arm3 == "General"
  ),
 orders = c(
   trialno = "Population",
   exclusion1 = "Ineligible",
   trialno = "Baseline Analysis",
   arm3 = "Cohort",
   lost_followup = "Excluded",
   mitt = "Followup Analysis"
   ),
 side_box = c(
   "exclusion1",
   "lost_followup"
   ),
 allocation = "arm3",
 cex = 0.6
)

knitr::knit_print(STROBEPlot2)

```

The flowcharts were separated into two components (@fig-strobe-cohort1 and @fig-strobe-cohort2) and exclusions noted for patients that were labelled as a procedure failure.

::: {#procedurefailure .callout-note}
A procedure failure is defined by a terminology in the clinical letter and/or a revision case booked that affects the previously operated joint region. The general definition for arthroplasty cases as per the NJRR was used, whereby removal of any significant component of the existing prosthesis with replacement was deemed a failure of the preceding arthroplasty. In the case of infections treated with long-term antibiotics, where the preceding surgical procedure was intended to treat an existing infection, these cases were failed. In cases where DAIR with antibiotic therapy was used for non-septic revisions or primary cases in which an infection has been detected, these have been noted as complications without failing the preceding treatment.
:::

::: {#progressionend .callout-note}
Treatment progression end is a term to denote that a treatment record is no longer being actively tracked by the registry team with respect to clinical outcomes or patient-reported outcomes. In this registry, the majority of cases are deceased, or are not contactable by the registry. A smaller number are stage 1 revisions that have transition to stage 2.
:::

Cumulative recruitment over time was plotted from Registry inception to the present.

```{r}
#| label: fig-progress
#| fig-cap: "Cumulative (non-excluded) record creation within COMPRESSOR from inception to present"


RecordCount <- dplyr::summarise(
  STROBEFlow3 |>  dplyr::filter(
    !(TreatmentStatus == "Archived")
    ),
  Count = n(),
  .by = PatientCreationDate
) |> arrange(
  PatientCreationDate
) |> dplyr::mutate(
  CumulativeCount = cumsum(Count),
  PatientCreationNum = as.numeric(PatientCreationDate)
  ) |> filter(
    !(PatientCreationDate < ymd("20160701"))
   
  )

cumulative_plot <- ggplot(RecordCount, aes(x = PatientCreationDate, y = CumulativeCount)) +
  geom_line(color = "#2c3e50", linewidth = 1) +
  # Add the spline fit with confidence interval
  geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs"), 
              color = "#e74c3c", linewidth = 1.2, se = TRUE, fill = "#e74c3c", alpha = 0.2) +
  geom_point(color = "#3498db", size = 1) +
  # Add labels and title
  labs(
    title = "Cumulative Patient Count Over Time",
    x = "Date",
    y = "Cumulative Number of Patients",
    #caption = "Data source: STROBEFlow3"
  ) +
  # Improve the theme and formatting
  theme_minimal() +
  theme(
    plot.title = element_text(face = "bold", size = 14),
    axis.title = element_text(face = "bold"),
    panel.grid.minor = element_blank()
  )

# Print the plot
knitr::knit_print(cumulative_plot)


```

The non-linear recruitment progression illustrated in @fig-progress denotes the distinct periods of operation of COMPRESSOR, combined with periods of retrospective chart review of key patient subgroups. The recent increase in recruitment has been facilitated by some reorganisation of the cohort structure of the registry. This has had the side effect of increasing the PROMs eligible pool at the relevant time points.

# Missingness and Compliance

## Baseline

The COMPRESSOR registry has a patchy history of baseline capture for relevant cohorts. A large proportion of those eligible for baseline capture have been due to their enrolment during the "active" periods of the registry, but not necessarily when the PROMs component of the registry has been active. This has created challenges in determining the eligibility of records for baseline capture.

```{r}
#| label: fig-baseline-miss
#| fig-cap: "Missingness of baseline PROMs by registry cohort"

samplesizeBase <- Snapshot |> 
  dplyr::filter(
    EligibleAtPreop == "Yes",
    SurgicalTreatment2 == "Surgical"
  ) |> 
  dplyr::count(RegistryCohortName, name = "n")

FigureMissBase <- gg_miss_fct(Snapshot |> dplyr::filter(
      EligibleAtPreop == "Yes",
      SurgicalTreatment2 == "Surgical"
      ) |> dplyr::select(
        VR12PCS_Preop,
        #PatientPosition,
        #TreatmentProvider, 
        RegistryCohortName
        ), 
      RegistryCohortName
      ) + scale_fill_viridis_c(
    option = "magma",  # or "inferno", "magma", "cividis"
    name = "% Missing",
    limits = c(0, 100)  # Set limits from 0% to 100%
  ) +
  geom_text(data = samplesizeBase, 
            aes(x = RegistryCohortName, y = 0.7, label = paste0("n=", n)),
            inherit.aes = FALSE,
            color = "white", 
            fontface = "bold",
            size = 3)

knitr::knit_print(FigureMissBase)

```

Missingness for baseline PROMs (represented by VR12, common to all cohorts) is captured in @fig-baseline-miss. Overall, the eligible pool remains small for many cohorts for baseline capture and response rates vary between cohorts.

## Patient-reported Outcomes by TimePoint

As the registry has been running for a relatively short period of time, the followup PROMs capture is best described as cross-sectional rather than longitudinal, in that there are few treatment records eligible for multiple timepoints in the follow up schedule.

```{r}
#| label: fig-vr12-miss
#| fig-cap: "Missingness of VR12 by Timepoint across all cohorts"



SampleSizeVR12 <- PROMVR12 |> 
  dplyr::filter(
    SurgicalTreatment2 == "Surgical",
    RegistryCohortName != "General"
  ) |> 
  dplyr::count(TimePoint, name = "n")

FigureVR12Miss <- gg_miss_fct(PROMVR12 |> dplyr::filter(
      !is.na(EligibleAt),
      SurgicalTreatment2 == "Surgical",
      RegistryCohortName != "General"
      ) |> dplyr::select(
        VR12PCS,
        TimePoint
        ), 
      TimePoint
      ) + scale_fill_viridis_c(
    option = "magma",  # or "inferno", "magma", "cividis"
    name = "% Missing",
    limits = c(0, 100)  # Set limits from 0% to 100%
  ) +
  geom_text(data = SampleSizeVR12, 
            aes(x = TimePoint, y = 0.7, label = paste0("n=", n)),
            inherit.aes = FALSE,
            color = "white", 
            fontface = "bold",
            size = 3)

knitr::knit_print(FigureVR12Miss)


```

The VR12 is the first score presented to patients at each timepoint, but is not presented at every timepoint for every cohort. The variable capture between timepoints illustrated in @fig-vr12-miss may reflect a number of underlying factors (cohort differences in engagement, mode of communication, whether it is the first attempt at followup).

::: {#eligibility .callout-note}
Eligibility for PROMs capture had to be calculated by determining the date on which the record was created, the date windows for each follow-up time-point and comparing these to the active periods of the registry operation. The eligible pool of treatments varies depending on how the active periods of PROMs collection line up with the follow-up schedule windows.
:::

```{r}
#| label: fig-msts-miss
#| fig-cap: "Missingness of MSTS by Timepoint cohort"




SampleSizeMSTS <- PROMMSTS |> 
  dplyr::filter(
    SurgicalTreatment2 == "Surgical",
    TimePoint != "Preop"
  ) |> 
  dplyr::count(TimePoint, name = "n")

FigureMSTSMiss <- gg_miss_fct(PROMMSTS |> dplyr::filter(SurgicalTreatment2 == "Surgical") |> dplyr::select(MSTSLower_Total,TimePoint), TimePoint) + scale_fill_viridis_c(
    option = "magma",  # or "inferno", "magma", "cividis"
    name = "% Missing",
    limits = c(0, 100)  # Set limits from 0% to 100%
  ) +
  geom_text(data = SampleSizeMSTS, 
            aes(x = TimePoint, y = 0.7, label = paste0("n=", n)),
            inherit.aes = FALSE,
            color = "white", 
            fontface = "bold",
            size = 3)

knitr::knit_print(FigureMSTSMiss)
```

The MSTS (@fig-msts-miss) is presented to TumourPelvis, TumourLowerLimb and ReinterventionTKA cohorts at every followup timepoint. The medium to long-term timepoints present ongoing challenges for re-establishing patient contact by email, with only some cases appearing on the GMRS followup list (N \~ 30 each for the 60months and 120months windows - see below).

```{r}
#| label: fig-hoos12-miss
#| fig-cap: "Missingness of HOOS12 by Timepoint in Surgical Cases of ReinterventionTHA"


SampleSizeHOOS12 <- PROMHOOS12 |> 
  dplyr::filter(
    SurgicalTreatment2 == "Surgical",
    RegistryCohortName == "ReinterventionTHA"
  ) |> 
  dplyr::count(TimePoint, name = "n")

FigureHOOSMiss <- gg_miss_fct(PROMHOOS12 |> dplyr::filter(SurgicalTreatment2 == "Surgical") |> dplyr::select(contains("HOOS12_Summary_TotalScore"), TimePoint), TimePoint) + scale_fill_viridis_c(
    option = "magma",  # or "inferno", "magma", "cividis"
    name = "% Missing",
    limits = c(0, 100)  # Set limits from 0% to 100%
  ) +
  geom_text(data = SampleSizeHOOS12, 
            aes(x = TimePoint, y = 0.7, label = paste0("n=", n)),
            inherit.aes = FALSE,
            color = "white", 
            fontface = "bold",
            size = 3)



knitr::knit_print(FigureHOOSMiss)
```

The HOOS12 (@fig-hoos12-miss) is presented only to the ReinterventionTHA cohort at all of the registry timepoints. The reduced capture preoperatively may be due to a number of underlying factors which are not yet clear. It may be that the cohort selection within the system has been too slow to capture patients prior to surgery or that patients are dropping out before filling in the HOOS (questionnaire fatigue) within the questionnaire pack.

# Clinical Results by Patient Group

This section presents results for each patient cohort in a consistent structure: who was treated, how the procedure held up over time, and how patients reported their quality of life and physical function. Each subsection opens with a plain-language summary of the key findings before the supporting data.

The VR12 is a *general* health questionnaire presented to *all* patients enrolled in the registry, covering two components: *Physical* and *Mental* health. Cohort-specific questionnaires (MSTS, HOOS-12, KOOS-12, BPI) are described where they appear.

## Cross-cohort Overview

::: {.callout-tip title="What this shows"}
- Physical health scores (VR12) vary considerably within and between cohorts at 12 months, reflecting the diversity of diagnoses and treatment complexity across the registry
- The Tumour Pelvis cohort shows a split pattern — a subgroup with relatively preserved physical function alongside a subgroup with substantially reduced scores
- Musculoskeletal function scores (MSTS) show a gradual decline over time in the Tumour Lower Limb group, consistent with the expected trajectory after limb-salvage surgery; the apparent improvement at 10 years is likely an artefact of survivor bias (see note below)
- These patterns are based on small, cross-sectional samples and should be treated as preliminary observations rather than definitive trends
:::

```{r}
#| label: tbl-vr12-cohort
#| tbl-cap: "Summary (median with interquartile ranges) of VR12 Physical Component Score at 12months by cohort"

TblVR12 <- Snapshot |> dplyr::filter(
  SurgicalTreatment2 == "Surgical",
  RegistryCohortName != "General",
  EligibleAtx12months == "Yes"
  ) |> dplyr::select(
    VR12PCS_12months,
    VR12MCS_12months,
    RegistryCohortName
  ) |> gtsummary::tbl_summary(
    by = "RegistryCohortName",
    label = list(
      VR12PCS_12months = "Physical Component",
      VR12MCS_12months = "Mental Component"
      )
  )

knitr::knit_print(TblVR12)

```

Between-cohort variability is observed for the Physical Component Score of the VR12 at 12 months (@tbl-vr12-cohort). Most notable is the wide spread of scores within all cohorts. The Tumour Pelvis cohort shows a bimodal pattern, with some patients reporting good physical function and others quite low — likely reflecting the heterogeneity of pelvic tumour presentations and surgical approaches.

::: {#survivorbias .callout-note collapse="true" title="Technical note: survivor bias"}
Survivor bias occurs when the patients remaining available for follow-up at a given timepoint are systematically different from those who have been lost — in this context, because patients with poorer function are more likely to have died or become unreachable before later timepoints. This means long-term PROMs may overestimate the functional status of the surviving cohort as a whole.
:::

## Tumour Pelvis

This cohort includes all cases presenting with abnormal growths directly interacting with the bones of the pelvis. While Ossis (3D-printed patient-specific implant) cases are included, they do not represent the majority of this cohort.

::: {.callout-tip title="Key findings"}
- Approximately 70% of primary cases remain free of procedure failure at 10 years; revision cases show substantially worse durability, with median survival around 6 years
- Patient-reported musculoskeletal function (MSTS) is captured at follow-up across multiple timepoints, though sample sizes remain small
- Physical health scores are highly variable within this cohort, consistent with the complexity and heterogeneity of pelvic tumour presentations
- Survival estimates should be interpreted cautiously given small numbers and competing risks from mortality (see note in Observations)
:::

```{r}
```

```{r}
```

```{r}
```

### Who were these patients?

```{r}
#| label: tbl-tp-demographics
#| tbl-cap: "Summary of demographics in the Tumour Pelvis cohort"

TableTPdemo <- gtsummary::tbl_summary(
  Snapshot |> dplyr::filter(
    RegistryCohortName == "TumourPelvis"
  ) |> dplyr::select(
    TreatmentType,
    SurgicalTreatment2,
    TreatmentStatus,
    DateInitialExamination,
    AgeAtInitialExam,
    AgeAtTreatment,
    Sex2,
    Retrospective,
    Planned,
    Hardware

   #EducationLevel_Preop,
   # DiagnosisPrimary
  ),
  by = "SurgicalTreatment2",
  missing = "no",
  statistic = list(
    DateInitialExamination ~ "{min} - {max}"
  )
)

knitr::knit_print(TableTPdemo)
```

### How did the procedure hold up?

```{r}
#| label: survival-prep-tp

SnapshotTP <- Snapshot |> dplyr::filter(
  RegistryCohortName == "TumourPelvis"
)

SnapshotTP2 <- SnapshotTP |> dplyr::mutate(
  Status = if_else(
    TreatmentStatus == "Failed",
    1,
    0
  ),
  EndDate = case_when(
    !is.na(DateStatusChange) ~ DateStatusChange,
    .default = coalesce(DateStatusChange, ymd(CurrentDate)),
    ),
  Duration = as.numeric(as.duration(interval(ymd(DateTreatment), ymd(EndDate))),"years"),
)

```

```{r}
#| label: fig-survtp
#| fig-cap: "Procedure survival for Tumour Pelvis cases by treatment type"


FigureSurvTP <- survfit2(Surv(Duration, Status) ~ TreatmentType2,
                        data = SnapshotTP2
                        ) |> ggsurvfit(linewidth = 1) +
  add_confidence_interval() +
  add_risktable() +
  add_quantile(y_value = 0.5, color = "gray50", linewidth = 0.75) +
  scale_ggsurvfit() +
  coord_cartesian(ylim = c(0, 1))

knitr::knit_print(FigureSurvTP)

```

```{r}
#| label: tbl-survTP
#| tbl-cap: "Tumour Pelvis: procedure survival at 1, 2, 5 and 10 years"

TableSurvTP <- tbl_survfit(
  survfit2(Surv(Duration, Status) ~ TreatmentType2, 
           data = SnapshotTP2),
  times = c(1,2,5,10),
  label_header = "**{time} Years**",
  label = "Procedure Survival",
  statistic = "{estimate} ({conf.low} - {conf.high})"
)

knitr::knit_print(TableSurvTP)

```

Primary cases achieved approximately 70% survival at 10 years (@fig-survtp, @tbl-survTP). Revision cases fared considerably worse, with median survival at approximately 6 years. Confidence intervals are wide, reflecting small sample sizes.

### What complications and reoperations were recorded?

```{r}
#| label: tbl-complic-tp
#| tbl-cap: "Complications and reoperations in the Tumour Pelvis cohort"

tbl_summary(
  ComplicWide |>
    dplyr::filter(RegistryCohortName == "TumourPelvis") |>
    dplyr::select(
      AnyComplication, AnySurgical,
      InfectionDeep, InfectionSuperficial, InfectionSyst,
      Wound, Thrombosis, Neurological, Dislocation,
      Loosening, Hardware, Fracture,
      TumourLocal, TumourSystemic,
      Reoperation, ImplantRemoval, Amputation, Mortality, Other
    ) |> dplyr::mutate(
  across(everything(), ~factor(.x, levels = c("No", "Yes")))
),
  label = list(
    AnyComplication      ~ "Any complication detected",
    AnySurgical          ~ "Required surgical management",
    InfectionDeep        ~ "Deep / periprosthetic infection",
    InfectionSuperficial ~ "Superficial infection",
    InfectionSyst        ~ "Systemic infection",
    Wound                ~ "Wound complication",
    Thrombosis           ~ "Thrombotic event",
    Neurological         ~ "Neurological complication",
    Dislocation          ~ "Instability / dislocation",
    Loosening            ~ "Component loosening",
    Hardware             ~ "Hardware issue",
    Fracture             ~ "Fracture",
    TumourLocal          ~ "Local tumour event",
    TumourSystemic       ~ "Systemic tumour event",
    Reoperation          ~ "Unplanned reoperation",
    ImplantRemoval       ~ "Implant removal",
    Amputation           ~ "Amputation",
    Mortality            ~ "Mortality",
    Other                ~ "Other"
  ),
  type    = list(everything() ~ "dichotomous"),
  value   = list(everything() ~ "Yes"),
  missing = "no"
) |>
  bold_labels()
```

### How did patients report their function?

```{r}
#| label: tbl-proms-tp
#| tbl-cap: "Patient-reported outcomes by timepoint: Tumour Pelvis"

tp_timepoints <- c("Preop", "3months", "6months", "12months", "24months", "60months", "120months")

tp_vr12 <- PROMVR12 |>
  dplyr::filter(RegistryCohortName == "TumourPelvis") 

tp_hoos12 <- PROMHOOS12 |>
  dplyr::filter(RegistryCohortName == "TumourPelvis") 

tp_msts <- PROMMSTS |>
  dplyr::filter(RegistryCohortName == "TumourPelvis") 

bind_rows(
  build_prom_table(tp_vr12, VR12PCS, "Physical Component", tp_timepoints),
  build_prom_table(tp_vr12, VR12MCS, "Mental Component", tp_timepoints),
  build_prom_table(tp_hoos12, HOOS12_Summary_TotalScore, "HOOS Total", tp_timepoints),
  build_prom_table(tp_msts, MSTSLower_Total, "MSTS", tp_timepoints)
) |>
  gt::gt(groupname_col = "Score", rowname_col = "TimePoint") |>
  gt::sub_missing(missing_text = "—") |>
  gt::tab_options(
    row_group.font.weight = "bold",
    row_group.padding     = gt::px(6)
  ) |>
  gt::tab_footnote(
    footnote  = "Median (Q1, Q3), n = non-missing observations",
    locations = gt::cells_title()
  )
```

MSTS scores for this cohort remain sparse (@tbl-proms-tp), making meaningful interpretation difficult at most timepoints. The available data are consistent with moderate functional limitation across the follow-up period, reflecting the surgical complexity of pelvic reconstruction.

## Tumour Lower Limb

This cohort includes all cases with abnormal growths directly involving the bones of the lower limb, excluding the pelvis. The majority of surgically treated cases in this cohort involve GMRS or MRH components.

::: {.callout-tip title="Key findings"}
- Primary cases show strong procedure durability, with approximately 60% surviving free of failure at 13 years
- Revision cases fare considerably worse, with 60% survival reached at approximately 2.5 years
- Pain levels (BPI) remain relatively low for primary cases across all timepoints, though small sample sizes limit conclusions for revision cases
- Musculoskeletal function (MSTS) shows a gradual decline over time for this cohort — consistent with the natural history of limb-salvage patients — with the caveat of survivor bias at longer timepoints
:::

### Who were these patients?

```{r}
#| label: tbl-tll-demographics
#| tbl-cap: "Summary of demographics in Tumour Lower Limb cohort"

TableTLLdemo <- gtsummary::tbl_summary(
  Snapshot |> dplyr::filter(
    RegistryCohortName == "TumourLowerLimb"
  ) |> dplyr::select(
    TreatmentType,
    SurgicalTreatment2,
    TreatmentStatus,
    DateInitialExamination,
    AgeAtInitialExam,
    Sex2,
    Retrospective,
    Hardware
    # EducationLevel_Preop,
    # DiagnosisPrimary
  ),
  by = "SurgicalTreatment2",
  missing = "no",
  statistic = list(
    DateInitialExamination ~ "{min} - {max}"
  )
)

knitr::knit_print(TableTLLdemo)
```

### How did the procedure hold up?

```{r}
#| label: survival-prep-tll

SnapshotTLL <- Snapshot |> dplyr::filter(
  RegistryCohortName == "TumourLowerLimb",
  SurgicalTreatment2 == "Surgical"
)

SnapshotTLL2 <- SnapshotTLL |> dplyr::mutate(
  Status = if_else(
    TreatmentStatus == "Failed",
    1,
    0
  ),
  EndDate = case_when(
    !is.na(DateStatusChange) ~ DateStatusChange,
    .default = coalesce(DateStatusChange, ymd(CurrentDate)),
    ),
  Duration = as.numeric(as.duration(interval(ymd(DateTreatment), ymd(EndDate))),"years"),
)

```

```{r}
#| label: fig-survtll
#| fig-cap: "Survival curve for Tumour Lower Limb cases by treatment type"


FigureSurvTLL <- survfit2(Surv(Duration, Status) ~ TreatmentType2,
                        data = SnapshotTLL2
                        ) |> ggsurvfit(linewidth = 1) +
  add_confidence_interval() +
  add_risktable() +
  add_quantile(y_value = 0.6, color = "gray50", linewidth = 0.75) +
  scale_ggsurvfit() +
  coord_cartesian(ylim = c(0.2, 1))

knitr::knit_print(FigureSurvTLL)

```

```{r}
#| label: tbl-survTLL
#| tbl-cap: "Summary of Tumour Lower Limb Procedure Survival"

TableSurvTLL <- tbl_survfit(
  survfit2(Surv(Duration, Status) ~ TreatmentType2, 
           data = SnapshotTLL2),
  times = c(1,2,5,10),
  label_header = "**{time} Years**",
  label = "Procedure Survival",
  statistic = "{estimate} ({conf.low} - {conf.high})"
)

knitr::knit_print(TableSurvTLL)

```

Primary cases show strong procedure durability, with 60% survival reached at approximately 13 years (@fig-survtll, @tbl-survTLL). Revision cases fare considerably worse, with 60% survival at approximately 2.5 years. Confidence intervals are wide at longer timepoints due to smaller at-risk numbers.

### What complications and reoperations were recorded?

```{r}
#| label: tbl-complic-tll
#| tbl-cap: "Complications and reoperations in the Tumour Lower Limb cohort"

tbl_summary(
  ComplicWide |>
    dplyr::filter(RegistryCohortName == "TumourLowerLimb") |>
    dplyr::select(
      AnyComplication, AnySurgical,
      InfectionDeep, InfectionSuperficial, InfectionSyst,
      Wound, Thrombosis, Neurological, Dislocation,
      Loosening, Hardware, Fracture,
      TumourLocal, TumourSystemic,
      Reoperation, ImplantRemoval, Amputation, Mortality, Other
    ) |> dplyr::mutate(
  across(everything(), ~factor(.x, levels = c("No", "Yes")))
),
  label = list(
    AnyComplication      ~ "Any complication detected",
    AnySurgical          ~ "Required surgical management",
    InfectionDeep        ~ "Deep / periprosthetic infection",
    InfectionSuperficial ~ "Superficial infection",
    InfectionSyst        ~ "Systemic infection",
    Wound                ~ "Wound complication",
    Thrombosis           ~ "Thrombotic event",
    Neurological         ~ "Neurological complication",
    Dislocation          ~ "Instability / dislocation",
    Loosening            ~ "Component loosening",
    Hardware             ~ "Hardware issue",
    Fracture             ~ "Fracture",
    TumourLocal          ~ "Local tumour event",
    TumourSystemic       ~ "Systemic tumour event",
    Reoperation          ~ "Unplanned reoperation",
    ImplantRemoval       ~ "Implant removal",
    Amputation           ~ "Amputation",
    Mortality            ~ "Mortality",
    Other                ~ "Other"
  ),
  type    = list(everything() ~ "dichotomous"),
  value   = list(everything() ~ "Yes"),
  missing = "no"
) |>
  bold_labels()
```

### How did patients report their function?

```{r}
#| label: tbl-proms-tll
#| tbl-cap: "Patient-reported outcomes by timepoint: Tumour Lower Limb"

tll_timepoints <- c("Preop", "3months", "6months", "12months", "24months", "60months", "120months")

tll_vr12 <- PROMVR12 |>
  dplyr::filter(RegistryCohortName == "TumourLowerLimb") 

tll_bpi <- PROMBPI |>
  dplyr::filter(RegistryCohortName == "TumourLowerLimb")

tll_msts <- PROMMSTS |>
  dplyr::filter(RegistryCohortName == "TumourLowerLimb")

bind_rows(
  build_prom_table(tll_vr12, VR12PCS, "Physical Component", tll_timepoints),
  build_prom_table(tll_vr12, VR12MCS, "Mental Component", tll_timepoints),
  build_prom_table(tll_bpi, BPISFSeverityScore, "Pain Severity", tll_timepoints),
  build_prom_table(tll_msts, MSTSLower_Total, "MSTS", tll_timepoints)
) |>
  gt::gt(groupname_col = "Score", rowname_col = "TimePoint") |>
  gt::sub_missing(missing_text = "—") |>
  gt::tab_options(
    row_group.font.weight = "bold",
    row_group.padding     = gt::px(6)
  ) |>
  gt::tab_footnote(
    footnote  = "Median (Q1, Q3), n = non-missing observations",
    locations = gt::cells_title()
  )
```

Direct comparison of pain severity between primary and revision cases is limited by small numbers (@tbl-proms-tll). For primary cases, reported pain levels remain relatively low across all timepoints up to 10 years, which is encouraging given the extent of surgery involved.

## GMRS-MRH (Stryker Components)

This subsection covers all surgical cases across cohorts in which Stryker GMRS or MRH components were used. Cases where both components were implanted are captured in the sponsor-specific reports rather than here. The General cohort within this group includes complex primary arthroplasties for knee deformity with concurrent osteoarthritis.

::: {.callout-tip title="Key findings"}
- 70% of primary cases remain free of procedure failure at approximately 10 years; for revision cases this threshold is reached at approximately 5 years
- Physical health (VR12) remains relatively stable across timepoints for the Reintervention TKA group; the Tumour Lower Limb group shows a more variable recovery from 6 months onward
- Musculoskeletal function (MSTS) differs between primary and revision cases in the Tumour Lower Limb group, though the cross-sectional nature of the data limits longitudinal interpretation
- PROMs capture rates and eligible numbers by surgical usage type are summarised in the registry operations section below
:::

### Who were these patients?

```{r}
#| label: tbl-gmrs-demographics
#| tbl-cap: "Summary of demographics in GMRS-MRH cohort"

TableGMRSdemo <- gtsummary::tbl_summary(
  Snapshot |> dplyr::filter(
    stringr::str_detect(str_to_lower(Hardware), "gmrs|mrh"),
    RegistryCohortName != "TumourUpperLimb",
    Planned != "Yes"
    #Surgeon == "RB"
  ) |> dplyr::select(
    RegistryCohortName,
    TreatmentType,
    SurgicalTreatment2,
    TreatmentStatus,
    DateInitialExamination,
    AgeAtInitialExam,
    Sex2,
    Hardware,
    Retrospective
    # EducationLevel_Preop,
    # DiagnosisPrimary
  ),
  by = "RegistryCohortName",
  missing = "no",
  statistic = list(
    DateInitialExamination ~ "{min} - {max}"
  )
) |>
  add_overall()

knitr::knit_print(TableGMRSdemo)
```

### How did the procedure hold up?

```{r}
#| label: survival-prep-gmrs

SnapshotGMRS <- Snapshot |> dplyr::filter(
  Hardware == "gmrs" | Hardware == "mrh",
  SurgicalTreatment2 == "Surgical",
  Planned != "Yes",
  Surgeon == "RB"
) |> dplyr::mutate(
  Status = if_else(
    TreatmentStatus == "Failed",
    1,
    0
  ),
  EndDate = case_when(
    !is.na(DateStatusChange) ~ DateStatusChange,
    .default = coalesce(DateStatusChange, ymd(CurrentDate)),
    ),
  Duration = as.numeric(as.duration(interval(ymd(DateTreatment), ymd(EndDate))),"years"),
)

```

```{r}
#| label: fig-survgmrs
#| fig-cap: "Survival curve for GMRS-MRH cases by treatment type"


FigureSurvGMRS <- survfit2(Surv(Duration, Status) ~ TreatmentType2,
                        data = SnapshotGMRS
                        ) |> ggsurvfit(linewidth = 1) +
  add_confidence_interval() +
  add_risktable() +
  add_quantile(y_value = 0.7, color = "gray50", linewidth = 0.75) +
  scale_ggsurvfit() +
  coord_cartesian(ylim = c(0.2, 1))

knitr::knit_print(FigureSurvGMRS)

```

```{r}
#| label: tbl-survgmrs
#| tbl-cap: "Summary of GMRS-MRH Procedure Survival"

TableSurvGMRS <- tbl_survfit(
  survfit2(Surv(Duration, Status) ~ TreatmentType2, 
           data = SnapshotGMRS),
  times = c(1,2,5,10),
  label_header = "**{time} Years**",
  label = "Procedure Survival",
  statistic = "{estimate} ({conf.low} - {conf.high})"
)

knitr::knit_print(TableSurvGMRS)

```

Approximately 70% of primary cases remain free of procedure failure at 10 years (@fig-survgmrs, @tbl-survgmrs). Revision cases show substantially reduced durability, with 70% survival at approximately 5 years. As with other cohorts, confidence intervals widen considerably at longer timepoints.

### What complications and reoperations were recorded?

```{r}
#| label: tbl-complic-gmrs
#| tbl-cap: "Complications and reoperations in the GMRS cohort"

tbl_summary(
  ComplicWide |>
    dplyr::filter(stringr::str_detect(str_to_lower(AnalysisLabel),"gmrs|mrh") & stringr::str_detect(str_to_lower(AnalysisLabel),"plan", negate = TRUE)) |>
    dplyr::select(
      AnyComplication, AnySurgical,
      InfectionDeep, InfectionSuperficial, InfectionSyst,
      Wound, Thrombosis, Neurological, Dislocation,
      Loosening, Hardware, Fracture,
      TumourLocal, TumourSystemic,
      Reoperation, ImplantRemoval, Amputation, Mortality, Other
    ) |> dplyr::mutate(
  across(everything(), ~factor(.x, levels = c("No", "Yes")))
),
  label = list(
    AnyComplication      ~ "Any complication detected",
    AnySurgical          ~ "Required surgical management",
    InfectionDeep        ~ "Deep / periprosthetic infection",
    InfectionSuperficial ~ "Superficial infection",
    InfectionSyst        ~ "Systemic infection",
    Wound                ~ "Wound complication",
    Thrombosis           ~ "Thrombotic event",
    Neurological         ~ "Neurological complication",
    Dislocation          ~ "Instability / dislocation",
    Loosening            ~ "Component loosening",
    Hardware             ~ "Hardware issue",
    Fracture             ~ "Fracture",
    TumourLocal          ~ "Local tumour event",
    TumourSystemic       ~ "Systemic tumour event",
    Reoperation          ~ "Unplanned reoperation",
    ImplantRemoval       ~ "Implant removal",
    Amputation           ~ "Amputation",
    Mortality            ~ "Mortality",
    Other                ~ "Other"
  ),
  type    = list(everything() ~ "dichotomous"),
  value   = list(everything() ~ "Yes"),
  missing = "no"
) |>
  bold_labels()
```

### How did patients report their function?

```{r}
#| label: tbl-proms-gmrs
#| tbl-cap: "Patient-reported outcomes by timepoint: GMRS-MRH"

gmrs_timepoints <- c("Preop", "3months", "6months", "12months", "24months", "60months", "120months")

gmrs_vr12 <- PROMVR12 |>
  dplyr::filter(stringr::str_detect(str_to_lower(AnalysisLabel),"gmrs|mrh") & stringr::str_detect(str_to_lower(AnalysisLabel),"plan", negate = TRUE)) 

gmrs_koos12 <- PROMKOOS12 |>
  dplyr::filter(stringr::str_detect(str_to_lower(AnalysisLabel),"gmrs|mrh") & stringr::str_detect(str_to_lower(AnalysisLabel),"plan", negate = TRUE)) 

gmrs_hoos12 <- PROMHOOS12 |>
  dplyr::filter(stringr::str_detect(str_to_lower(AnalysisLabel),"gmrs|mrh") & stringr::str_detect(str_to_lower(AnalysisLabel),"plan", negate = TRUE)) 

gmrs_bpi <- PROMBPI |>
  dplyr::filter(stringr::str_detect(str_to_lower(AnalysisLabel),"gmrs|mrh") & stringr::str_detect(str_to_lower(AnalysisLabel),"plan", negate = TRUE)) 

gmrs_msts <- PROMMSTS |>
  dplyr::filter(stringr::str_detect(str_to_lower(AnalysisLabel),"gmrs|mrh") & stringr::str_detect(str_to_lower(AnalysisLabel),"plan", negate = TRUE))

bind_rows(
  build_prom_table(gmrs_vr12, VR12PCS, "Physical Component", gmrs_timepoints),
  build_prom_table(gmrs_vr12, VR12MCS, "Mental Component", gmrs_timepoints),
  build_prom_table(gmrs_koos12, KOOS12_Summary_TotalScore, "KOOS TOtal", gmrs_timepoints),
  build_prom_table(gmrs_hoos12, HOOS12_Summary_TotalScore, "HOOS TOtal", gmrs_timepoints),
  build_prom_table(gmrs_bpi, BPISFSeverityScore, "Pain Severity", gmrs_timepoints),
  build_prom_table(gmrs_msts, MSTSLower_Total, "MSTS", gmrs_timepoints)
) |>
  gt::gt(groupname_col = "Score", rowname_col = "TimePoint") |>
  gt::sub_missing(missing_text = "—") |>
  gt::tab_options(
    row_group.font.weight = "bold",
    row_group.padding     = gt::px(6)
  ) |>
  gt::tab_footnote(
    footnote  = "Median (Q1, Q3), n = non-missing observations",
    locations = gt::cells_title()
  )


```

Physical health scores remain difficult to interpret at this stage due to small, cross-sectional samples. The Reintervention TKA group shows relative stability across timepoints, while the Tumour Lower Limb group shows a more variable pattern from 6 months onwards — consistent with the more complex recovery trajectory expected after limb-salvage surgery.

Musculoskeletal function scores for the GMRS-MRH subgroup (@tbl-proms-gmrs) are limited by small cross-sectional samples. Within the Tumour Lower Limb cohort, differences between primary and revision cases are apparent even with the available data, with revision cases showing lower scores at most timepoints.

### PROMs Capture Progress Against Targets

The table below tracks how many patients are eligible and how many have returned questionnaires at each follow-up timepoint, broken down by surgical usage category. Target case numbers are drawn from the study [Synopsis](https://docs.google.com/document/d/1u2V1-J337kgyolVJxOBVcHWOvEHkMGrmdLkX4it4mh8/edit).

::: {#synopsis-targets}
Complex primary TKA: 67 cases

Revision TKA: 79 cases

Distal femoral reconstruction: 53 cases

Proximal tibial reconstruction: 55 cases

Proximal femoral reconstruction: 78 cases

Total femoral replacement: As available

Femoral diaphyseal defect reconstruction: As available
:::

```{r}

SnapshotGMRSPROM <- SnapshotGMRS |> dplyr::select(
  TreatmentID,
  TreatmentType,
  TreatmentActivity,
  RegistryCohortName,
  DiagnosisRawPrelim,
  starts_with("Eligible")
) |> dplyr::mutate(
  across(everything(), ~replace_na(.x, "No")),
  ComplexTKA = if_else((RegistryCohortName == "General") | (TreatmentType == "Primary" & stringr::str_detect(str_to_lower(DiagnosisRawPrelim),"knee|mrh")),"ComplexPrimaryTKA",NA_character_),
    RevisionTKA = if_else((RegistryCohortName == "ReinterventionTKA") | (stringr::str_detect(str_to_lower(TreatmentType), "revision") & stringr::str_detect(str_to_lower(DiagnosisRawPrelim),"knee|mrh")),"RevisionTKA",NA_character_),
  ProximalFemur = if_else(stringr::str_detect(str_to_lower(DiagnosisRawPrelim),"proximal fem"), "ProximalFemur",NA_character_
  ),
  Intercalary = if_else(stringr::str_detect(str_to_lower(DiagnosisRawPrelim),"intercalary"), "Intercalary",NA_character_
  ),
 TotalFemur = if_else(stringr::str_detect(str_to_lower(DiagnosisRawPrelim),"total fem"),  "TotalFemur",NA_character_
  ),
  DistalFemur = if_else(stringr::str_detect(str_to_lower(DiagnosisRawPrelim),"distal fem"),  "DistalFemur",NA_character_
  ),
   ProximalTibia = if_else(stringr::str_detect(str_to_lower(DiagnosisRawPrelim),"prox.*tibia"),  "ProximalTibia",NA_character_
  )
  ) |> pivot_longer(
    cols = ComplexTKA:ProximalTibia,
    names_to = "UsageComponent",
    values_to = "Usage",
    values_drop_na = TRUE
  ) |> pivot_longer(
    cols = starts_with("EligibleAt"),
    names_to = "TimePoint",
    values_to = "Eligibility",
    values_drop_na = TRUE,
    names_prefix = "EligibleAt"
  ) |> filter(
    TimePoint != "Intraop"
    ) |> mutate(
    TimePoint = stringr::str_remove(TimePoint,"x"),
    TimePoint = forcats::fct(TimePoint, levels = c("Preop","3months","6months","12months","24months","60months","120months"))
  ) |> unite(
    "TimePointID",
    c("TreatmentID","TimePoint"),
    sep = ".",
    na.rm = FALSE,
    remove = FALSE
    ) |> left_join(
      PROMVR12 |> dplyr::select(TimePointID,VR12PCS),
      by = "TimePointID"
    ) |> left_join(
    PROMMSTS |> dplyr::select(TimePointID,MSTSLower_Total),
    by = "TimePointID"
  ) |> left_join(
    PROMHOOS12 |> dplyr::select(TimePointID,HOOS12_Summary_TotalScore),
    by = "TimePointID"
  ) |> left_join(
    PROMKOOS12 |> dplyr::select(TimePointID,KOOS12_Summary_TotalScore),
    by = "TimePointID"
  ) |> left_join(
    PROMBPI |> dplyr::select(TimePointID,BPISFSeverityScore),
    by = "TimePointID"
  ) |> naniar::add_shadow(VR12PCS,HOOS12_Summary_TotalScore,KOOS12_Summary_TotalScore,BPISFSeverityScore,MSTSLower_Total)




```

```{r}

#| label: tbl-promeligible
#| tbl-cap: "Summary of PROMs Eligibility by Timepoint for GMRS | MRH cases"

eligible_summary <- SnapshotGMRSPROM |>
  summarise(
    n_eligible = sum(Eligibility == "Yes", na.rm = TRUE),
    .by = c(TimePoint, Usage)
  ) |> arrange(TimePoint)

# Main body: n_eligible per TimePoint x Usage
main_table <- eligible_summary |>
  mutate(cell = as.character(n_eligible)) |>
  select(TimePoint, Usage, cell) |>
  tidyr::pivot_wider(
    names_from = Usage,
    values_from = cell
  ) |>
  rename(` ` = TimePoint)

# Footer row 1: distinct TreatmentIDs per Usage category
footer_total <- SnapshotGMRSPROM |>
  summarise(
    n_total = n_distinct(TreatmentID),
    .by = Usage
  ) |>
  mutate(cell = as.character(n_total)) |>
  select(Usage, cell) |>
  tidyr::pivot_wider(
    names_from = Usage,
    values_from = cell
  ) |>
  mutate(` ` = "N Cases (Total)", .before = 1)

# Footer row 2: distinct TreatmentIDs where TreatmentActivity == "Active"
footer_active <- SnapshotGMRSPROM |>
  filter(TreatmentActivity == "Active") |>
  summarise(
    n_active = n_distinct(TreatmentID),
    .by = Usage
  ) |>
  mutate(cell = as.character(n_active)) |>
  select(Usage, cell) |>
  tidyr::pivot_wider(
    names_from = Usage,
    values_from = cell
  ) |>
  mutate(` ` = "N Cases (Active)", .before = 1)

# Bind and render
bind_rows(main_table, footer_total, footer_active) |>
  gt::gt(rowname_col = " ") |>
  gt::tab_header(
    title = "Summary of PROMs Eligibility by Timepoint for GMRS | MRH cases"
  ) |>
  gt::tab_style(
    style = gt::cell_text(weight = "bold"),
    locations = gt::cells_stub(rows = c("N Cases (Total)", "N Cases (Active)"))
  ) |>
  gt::opt_stylize(style = 1) |>
  gt::opt_table_font(font = gt::google_font("Inter"))

```

```{r}
#| label: tbl-promcapture1
#| tbl-cap: "Summary of PROMs Capture by Timepoint for GMRS | MRH cases"
# N returned per TimePoint x Usage (any one PROM returned)
returned_summary <- SnapshotGMRSPROM |>
  mutate(
    returned = VR12PCS_NA == "!NA" |
               MSTSLower_Total_NA == "!NA" |
               BPISFSeverityScore_NA == "!NA" |
               HOOS12_Summary_TotalScore_NA == "!NA" |
               KOOS12_Summary_TotalScore_NA == "!NA"
  ) |>
  summarise(
    n_returned = sum(returned, na.rm = TRUE),
    .by = c(TimePoint, Usage)
  ) |> arrange(TimePoint)

# Join to eligible counts to calculate percentage
returned_table <- returned_summary |>
  left_join(
    eligible_summary,
    by = c("TimePoint", "Usage")
  ) |>
  mutate(
    pct = round((n_returned / n_eligible) * 100, 1),
    cell = glue::glue("{n_returned} ({pct}%)")
  ) |>
  select(TimePoint, Usage, cell) |>
  tidyr::pivot_wider(
    names_from = Usage,
    values_from = cell
  ) |>
  rename(` ` = TimePoint)

# Render
returned_table |>
  gt::gt(rowname_col = " ") |>
  gt::tab_header(
    title = "Summary of Returned PROMs by Timepoint for GMRS | MRH cases"
  ) |>
  gt::opt_stylize(style = 1) |>
  gt::opt_table_font(font = gt::google_font("Inter"))
```

```{r}

```

```{r}

# PROM Usage Summary Table Generator
# ============================================================
# Strategy:
#  - TimePoint capture = combined across PROMs (TRUE if any of the
#    three PROMs was captured for that TreatmentID at that TimePoint).
#  - Usage is NOT constant per TreatmentID: the same TreatmentID can
#    appear under multiple Usage tags (independent of TimePoint).
#    -> By-Usage columns are built from distinct TreatmentID x Usage
#       combinations, so a multi-tagged treatment correctly contributes
#       to every Usage column it belongs to.
#    -> The Overall column is built from distinct TreatmentID only, so
#       a multi-tagged treatment is counted once (not once per tag).
# ============================================================
 
library(tidyverse)
library(gtsummary)
library(epoxy)
library(naniar)
 
# Builds an "Overall" column (from deduplicated TreatmentID-level data)
# merged side-by-side with a by-Usage breakdown (from TreatmentID x Usage
# level data), for a given set of variables.
build_overall_and_by_usage <- function(overall_data, usage_data, vars, ...) {
 
  tbl_overall <- gtsummary::tbl_summary(
    overall_data,
    include = all_of(vars),
    missing = "no",
    ...
  ) |>
    modify_header(stat_0 ~ "**Overall**, N = {N}")
 
  tbl_by_usage <- gtsummary::tbl_summary(
    usage_data,
    include = all_of(vars),
    by = Usage,
    missing = "no",
    ...
  )
 
  gtsummary::tbl_merge(
    list(tbl_overall, tbl_by_usage),
    tab_spanner = FALSE
  )
}
 
generate_usage_summaries <- function(data) {
 
  epoxy("Flagging PROM capture at the row level (TreatmentID x TimePoint)")
 
  data_flagged <- data |>
    mutate(
      PROM_captured_row = (MSTSLower_Total_NA == "!NA") |
        (HOOS12_Summary_TotalScore_NA == "!NA") |
        (KOOS12_Summary_TotalScore_NA == "!NA")
    )
 
  epoxy("Collapsing to one row per TreatmentID (drives the Overall column)")
 
  treatment_level <- data_flagged |>
    group_by(TreatmentID) |>
    summarise(
      RegistryCohortName = first(RegistryCohortName),
      Preop       = any(TimePoint == "Preop"     & PROM_captured_row),
      `3months`   = any(TimePoint == "3months"   & PROM_captured_row),
      `6months`   = any(TimePoint == "6months"   & PROM_captured_row),
      `12months`  = any(TimePoint == "12months"  & PROM_captured_row),
      `24months`  = any(TimePoint == "24months"  & PROM_captured_row),
      `60months`  = any(TimePoint == "60months"  & PROM_captured_row),
      `120months` = any(TimePoint == "120months" & PROM_captured_row),
      MSTSLower_Total_NA           = any(MSTSLower_Total_NA == "!NA"),
      HOOS12_Summary_TotalScore_NA = any(HOOS12_Summary_TotalScore_NA == "!NA"),
      KOOS12_Summary_TotalScore_NA = any(KOOS12_Summary_TotalScore_NA == "!NA"),
      VR12PCS_NA = any(VR12PCS_NA == "!NA"),
      BPISFSeverityScore_NA = any(BPISFSeverityScore_NA == "!NA"), 
      .groups = "drop"
    )
 
  epoxy("treatment_level has {nrow(treatment_level)} rows (expected {n_distinct(data$TreatmentID)} distinct TreatmentID)")
  stopifnot(nrow(treatment_level) == n_distinct(data$TreatmentID))
 
  epoxy("Building distinct TreatmentID x Usage combinations (drives the by-Usage columns)")
 
  usage_lookup <- data |>
    distinct(TreatmentID, Usage)
 
  epoxy("{nrow(usage_lookup)} TreatmentID x Usage rows from {n_distinct(usage_lookup$TreatmentID)} distinct TreatmentID")
 
  usage_level <- usage_lookup |>
    left_join(treatment_level, by = "TreatmentID")
 
  epoxy("Building RegistryCohortName table (Overall + by Usage)")
 
  tbl_cohort <- build_overall_and_by_usage(
    overall_data = treatment_level,
    usage_data = usage_level,
    vars = "RegistryCohortName"
  )
 
  epoxy("Building TimePoint table (Overall + by Usage, PROM captured per timepoint)")
 
  timepoint_vars <- c("Preop", "3months", "6months", "12months", "24months", "60months", "120months")
 
  tbl_timepoint <- build_overall_and_by_usage(
    overall_data = treatment_level,
    usage_data = usage_level,
    vars = timepoint_vars,
    type = list(everything() ~ "dichotomous"),
    value = list(everything() ~ "TRUE")
  )
 
  epoxy("Building PROM captured (ever) table (Overall + by Usage)")
 
  captured_vars <- c("MSTSLower_Total_NA", "HOOS12_Summary_TotalScore_NA", "KOOS12_Summary_TotalScore_NA", "VR12PCS_NA","BPISFSeverityScore_NA")
 
  tbl_captured <- build_overall_and_by_usage(
    overall_data = treatment_level,
    usage_data = usage_level,
    vars = captured_vars,
    type = list(everything() ~ "dichotomous"),
    value = list(everything() ~ "TRUE"),
    label = list(
      MSTSLower_Total_NA = "MSTS Captured",
      HOOS12_Summary_TotalScore_NA = "HOOS12 Captured",
      KOOS12_Summary_TotalScore_NA = "KOOS12 Captured",
      VR12PCS_NA = "VR12 Captured",
      BPISFSeverityScore_NA = "BPI Captured"
    )
  )
 
  epoxy("Stacking sub-tables")
 
  stacked_table <- tbl_stack(
    list(tbl_cohort, tbl_timepoint, tbl_captured),
    quiet = TRUE
  )
 
  return(stacked_table)
}
 
# Usage example:
# stacked_summary <- generate_usage_summaries(SnapshotGMRSPROM)

```





```{r}
#| label: tbl-promcapture2
#| tbl-cap: "Summary of PROMs Capture by Timepoint for GMRS | MRH cases"
# Usage example:


GMRSstacked_summary <- generate_usage_summaries(SnapshotGMRSPROM)

knitr::knit_print(GMRSstacked_summary)
```

### Generate Initial List for Review

A master list is created for case-by-case review in an accessible format.

```{r}
GMRSExport <- Snapshot |> dplyr::filter(
  stringr::str_detect(str_to_lower(Hardware), "gmrs|mrh"),
    RegistryCohortName != "TumourUpperLimb",
    Planned != "Yes",
  Surgeon == "RB"
) |> dplyr::select(
  TreatmentID,
  PatientID,
  CombID,
  AgeAtInitialExam,
  Surgeon,
  Provider,
  DateTreatment,
  AffectedSide,
  RegistryStatus,
  RegistryStatusNotes,
  DateRegistryStatus,
  TreatmentType2,
  Retrospective,
  TreatmentStatus,
  TreatmentStatusNotes,
  DateStatusChange,
  DiagnosisRawFinal,
  DiagnosisRawPrelim,
  RegistryCohortName,
  AnalysisLabel,
  SurgicalTreatment2,
  Hardware,
  starts_with("Eligible")
) 


GMRSIndex <- GMRSExport |> dplyr::group_by(
  CombID
) |> slice_min(DateTreatment, n = 1, with_ties = FALSE) |>
  ungroup() |> left_join(
    PatientTable |> select(
      PatientID,
      FirstName,
      LastName,
      DateOfBirth,
      Phone
    ),
    by = "PatientID") |> relocate(
      all_of(c(
      "PatientID",
      "FirstName",
      "LastName",
      "DateOfBirth",
      "Phone"
      )
      )
    )

GMRSSubseq <- Snapshot |> filter(
  CombID %in% GMRSIndex$CombID,
  ! TreatmentID %in% GMRSIndex$TreatmentID
  ) |>
  left_join(
    GMRSIndex |> select(CombID, DateTreatment) |> rename(IndexDate = DateTreatment),
    by = "CombID"
  ) |>
  filter(DateTreatment > IndexDate) |>
  select(
    TreatmentID,
    PatientID,
    CombID,
    AgeAtInitialExam,
    Surgeon,
    Provider,
    DateTreatment,
    AffectedSide,
    RegistryStatus,
    RegistryStatusNotes,
    DateRegistryStatus,
    TreatmentType2,
    Retrospective,
    TreatmentStatus,
    TreatmentStatusNotes,
    DateStatusChange,
    DiagnosisRawFinal,
    DiagnosisRawPrelim,
    RegistryCohortName,
    AnalysisLabel,
    SurgicalTreatment2,
    Hardware,
    starts_with("Eligible")
  ) |>
  group_by(CombID) |>
  arrange(DateTreatment, .by_group = TRUE)

```

```{r}

GMRSCheck <- GMRSIndex |> filter(
  TreatmentStatus == "Ongoing"
)

ComplicGMRS <- ComplicTable |> filter(
  TreatmentID %in% GMRSExport$TreatmentID
)
```

```{r}
#| label: write-export
#| execute: false



googlesheets4::write_sheet(GMRSIndex, ss = "https://docs.google.com/spreadsheets/d/1SwFDscBu0jWhNgY6_LcFVfceDa70FXdbr3veRuDJRDY/edit", sheet = "GMRSIndex")

googlesheets4::write_sheet(GMRSSubseq, ss = "https://docs.google.com/spreadsheets/d/1SwFDscBu0jWhNgY6_LcFVfceDa70FXdbr3veRuDJRDY/edit", sheet = "GMRSSubseq")

googlesheets4::write_sheet(GMRSCheck, ss = "https://docs.google.com/spreadsheets/d/1SwFDscBu0jWhNgY6_LcFVfceDa70FXdbr3veRuDJRDY/edit", sheet = "GMRSCheck")

googlesheets4::write_sheet(ComplicGMRS, ss = "https://docs.google.com/spreadsheets/d/1SwFDscBu0jWhNgY6_LcFVfceDa70FXdbr3veRuDJRDY/edit", sheet = "GMRSComplic")
```

## 3D-Printed Pelvic Implant (Ossis-ZimmerBiomet)

This subsection covers all surgical cases across cohorts in which Ossis components were used.

::: {.callout-tip title="Key findings"}
- 70% of primary cases remain free of procedure failure at approximately 10 years; for revision cases this threshold is reached at approximately 5 years
- Physical health (VR12) remains relatively stable across timepoints for the Reintervention TKA group; the Tumour Lower Limb group shows a more variable recovery from 6 months onward
- Musculoskeletal function (MSTS) differs between primary and revision cases in the Tumour Lower Limb group, though the cross-sectional nature of the data limits longitudinal interpretation
- PROMs capture rates and eligible numbers by surgical usage type are summarised in the registry operations section below
:::

### Who were these patients?

```{r}
#| label: tbl-3dpi-demographics
#| tbl-cap: "Summary of demographics in 3DPI cohort"

Table3dpidemo <- gtsummary::tbl_summary(
  Snapshot |> dplyr::filter(
    stringr::str_detect(str_to_lower(Hardware), "ossis"),
    RegistryCohortName != "TumourUpperLimb",
    #Surgeon == "RB",
    Planned != "Yes",
  ) |> dplyr::select(
    RegistryCohortName,
    TreatmentType2,
    TreatmentStatus,
    DateInitialExamination,
    AgeAtInitialExam,
    Sex2,
    Retrospective
    # EducationLevel_Preop,
    # DiagnosisPrimary
  ),
  by = "RegistryCohortName",
  missing = "no",
  statistic = list(
    DateInitialExamination ~ "{min} - {max}"
  )
) |>
  add_overall()

knitr::knit_print(Table3dpidemo)
```

### How did the procedure hold up?

```{r}
#| label: survival-prep-3dpi

Snapshot3DPI <- Snapshot |> dplyr::filter(
  Hardware == "ossis",
  SurgicalTreatment2 == "Surgical",
  Planned != "Yes",
  #Surgeon == "RB"
) |> dplyr::mutate(
  Status = if_else(
    TreatmentStatus == "Failed",
    1,
    0
  ),
  EndDate = case_when(
    !is.na(DateStatusChange) ~ DateStatusChange,
    .default = coalesce(DateStatusChange, ymd(CurrentDate)),
    ),
  Duration = as.numeric(as.duration(interval(ymd(DateTreatment), ymd(EndDate))),"years"),
)

```

```{r}
#| label: fig-surv3dpi
#| fig-cap: "Survival curve for 3DPI cases by treatment type"


FigureSurv3DPI <- survfit2(Surv(Duration, Status) ~ TreatmentType2,
                        data = Snapshot3DPI
                        ) |> ggsurvfit(linewidth = 1) +
  add_confidence_interval() +
  add_risktable() +
  add_quantile(y_value = 0.7, color = "gray50", linewidth = 0.75) +
  scale_ggsurvfit() +
  coord_cartesian(ylim = c(0.2, 1))

knitr::knit_print(FigureSurv3DPI)

```

```{r}
#| label: tbl-surv3dpi
#| tbl-cap: "Summary of 3DPI Procedure Survival"

TableSurv3DPI <- tbl_survfit(
  survfit2(Surv(Duration, Status) ~ TreatmentType2, 
           data = Snapshot3DPI),
  times = c(1,2,5,10),
  label_header = "**{time} Years**",
  label = "Procedure Survival",
  statistic = "{estimate} ({conf.low} - {conf.high})"
)

knitr::knit_print(TableSurv3DPI)

```

Approximately 80% of primary cases remain free of procedure failure at 10 years (@fig-surv3dpi, @tbl-surv3dpi). Revision cases show substantially reduced durability, with 70% survival at approximately 5 years. As with other cohorts, confidence intervals widen considerably at longer timepoints.

### What complications and reoperations were recorded?

```{r}
#| label: tbl-complic-3dpi
#| tbl-cap: "Complications and reoperations in the 3DPI cohort"

tbl_summary(
  ComplicWide |>
    dplyr::filter(stringr::str_detect(str_to_lower(AnalysisLabel),"ossis") & stringr::str_detect(str_to_lower(AnalysisLabel),"plan", negate = TRUE)) |>
    dplyr::select(
      AnyComplication, AnySurgical,
      InfectionDeep, InfectionSuperficial, InfectionSyst,
      Wound, Thrombosis, Neurological, Dislocation,
      Loosening, Hardware, Fracture,
      TumourLocal, TumourSystemic,
      Reoperation, ImplantRemoval, Amputation, Mortality, Other
    ) |> dplyr::mutate(
  across(everything(), ~factor(.x, levels = c("No", "Yes")))
),
  label = list(
    AnyComplication      ~ "Any complication detected",
    AnySurgical          ~ "Required surgical management",
    InfectionDeep        ~ "Deep / periprosthetic infection",
    InfectionSuperficial ~ "Superficial infection",
    InfectionSyst        ~ "Systemic infection",
    Wound                ~ "Wound complication",
    Thrombosis           ~ "Thrombotic event",
    Neurological         ~ "Neurological complication",
    Dislocation          ~ "Instability / dislocation",
    Loosening            ~ "Component loosening",
    Hardware             ~ "Hardware issue",
    Fracture             ~ "Fracture",
    TumourLocal          ~ "Local tumour event",
    TumourSystemic       ~ "Systemic tumour event",
    Reoperation          ~ "Unplanned reoperation",
    ImplantRemoval       ~ "Implant removal",
    Amputation           ~ "Amputation",
    Mortality            ~ "Mortality",
    Other                ~ "Other"
  ),
  type    = list(everything() ~ "dichotomous"),
  value   = list(everything() ~ "Yes"),
  missing = "no"
) |>
  bold_labels()
```

### How did patients report their function?

```{r}
#| label: tbl-proms-3dpi
#| tbl-cap: "Patient-reported outcomes by timepoint: 3DPI"

ossis_timepoints <- c("Preop", "3months", "6months", "12months", "24months", "60months", "120months")

ossis_vr12 <- PROMVR12 |>
  dplyr::filter(stringr::str_detect(str_to_lower(AnalysisLabel),"ossis") & stringr::str_detect(str_to_lower(AnalysisLabel),"plan", negate = TRUE)) 

ossis_koos12 <- PROMKOOS12 |>
  dplyr::filter(stringr::str_detect(str_to_lower(AnalysisLabel),"ossis") & stringr::str_detect(str_to_lower(AnalysisLabel),"plan", negate = TRUE)) 

ossis_hoos12 <- PROMHOOS12 |>
  dplyr::filter(stringr::str_detect(str_to_lower(AnalysisLabel),"ossis") & stringr::str_detect(str_to_lower(AnalysisLabel),"plan", negate = TRUE)) 

ossis_bpi <- PROMBPI |>
  dplyr::filter(stringr::str_detect(str_to_lower(AnalysisLabel),"ossis") & stringr::str_detect(str_to_lower(AnalysisLabel),"plan", negate = TRUE)) 

ossis_msts <- PROMMSTS |>
  dplyr::filter(stringr::str_detect(str_to_lower(AnalysisLabel),"ossis") & stringr::str_detect(str_to_lower(AnalysisLabel),"plan", negate = TRUE))

bind_rows(
  build_prom_table(ossis_vr12, VR12PCS, "Physical Component", ossis_timepoints),
  build_prom_table(ossis_vr12, VR12MCS, "Mental Component", ossis_timepoints),
  build_prom_table(ossis_koos12, KOOS12_Summary_TotalScore, "KOOS TOtal", ossis_timepoints),
  build_prom_table(ossis_hoos12, HOOS12_Summary_TotalScore, "HOOS TOtal", ossis_timepoints),
  build_prom_table(ossis_bpi, BPISFSeverityScore, "Pain Severity", ossis_timepoints),
  build_prom_table(ossis_msts, MSTSLower_Total, "MSTS", ossis_timepoints)
) |>
  gt::gt(groupname_col = "Score", rowname_col = "TimePoint") |>
  gt::sub_missing(missing_text = "—") |>
  gt::tab_options(
    row_group.font.weight = "bold",
    row_group.padding     = gt::px(6)
  ) |>
  gt::tab_footnote(
    footnote  = "Median (Q1, Q3), n = non-missing observations",
    locations = gt::cells_title()
  )


```

Physical health scores remain difficult to interpret at this stage due to small, cross-sectional samples (@tbl-proms-3dpi). The Reintervention TKA group shows relative stability across timepoints, while the Tumour Lower Limb group shows a more variable pattern from 6 months onwards — consistent with the more complex recovery trajectory expected after limb-salvage surgery.
